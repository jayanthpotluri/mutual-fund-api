...
const createMutualFundTableSql = 'CREATE TABLE IF NOT EXISTS mutualfund (ID INT AUTO_INCREMENT PRIMARY KEY, MutualFundName VARCHAR(255), SchemeName VARCHAR(255), ISIN VARCHAR(255), LastUpdated DATETIME)';
...
const createNavDataTableSql = 'CREATE TABLE IF NOT EXISTS navdata (ID INT AUTO_INCREMENT PRIMARY KEY, NAV VARCHAR(255), DATE VARCHAR(255), MutualFundID INT, FOREIGN KEY (MutualFundID) REFERENCES mutualfund(ID))';
...
async function insertData(rows, mfId) {
  try {
    ...
    const insertSql = 'INSERT INTO navdata (NAV, DATE, MutualFundID) VALUES ?';
    const formattedRows = rows.map((row) => [parseFloat(row[0]), row[1], mfId]); // Formatted into an array of 3 elements - NAV, DATE, MutualFundID
    await new Promise((resolve, reject) => {
    ...
    // Update the LastUpdated column in the mutualfund table
    const updateSql = 'UPDATE mutualfund SET LastUpdated = NOW() WHERE ID = ?';
    await new Promise((resolve, reject) => {

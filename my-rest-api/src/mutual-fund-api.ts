...
app.get('/:mfId/:scId/:fdate/:tdate', async (req, res) => {
  try {
    ...
    const mfId: string = req.params.mfId; // MutualFund ID
    const scId: string = req.params.scId; // Scheme ID
    const fdate: string = req.params.fdate; // Date range - From date
    const tdate: string = req.params.tdate; // Date range - To date
    
    const url: string = 'https://www.amfiindia.com/modules/NavHistoryPeriod';
    const headers = { 
      'Accept': '*/*', 
    ...
    cron.schedule('*/10 * * * * *',async()=>{
      const data: string = `mfID=${mfId}&scID=${scId}&fDate=${fdate}&tDate=${tdate}`; // Building the request
...
async function insertData(rows: string[][], mfId: string) {
  try {
    ...
    await new Promise<void>((resolve, reject) => { // Promise<void> needed to execute resolve()
      const checkSql = 'SELECT lastUpdated FROM mutualfund WHERE mfID = ?';
      db.query(checkSql, [mfId], function (err: mysql.MysqlError | null, result: any) {
        if (err) {
          console.error('MySQL select query error:', err);
          reject(err);
        } else {
